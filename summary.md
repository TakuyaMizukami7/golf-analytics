...
## 4. 重要なファイル構成
- `src/app/page.tsx`: メインページ。ダッシュボードとAIコーチを左右（モバイルは上下）に配置する2カラムレイアウトで、`analysisData` 状態を管理。
- `src/components/Dashboard.tsx`: AIからの実際の分析結果（良い点・改善点・ドリル）を表示するダッシュボードコンポーネント。未分析時のEmpty State表示機能や、キャラクターが吹き出しでコメントするデザインの天気・ゴルフ日和判定ウィジェットを内包。各データカードクリックでチャットへプロンプトを送信する連携機能を持つ。
...
- `src/app/api/chat/route.ts`: GCSからのダウンロード、Gemini APIへのファイルアップロード、チャット生成、検索ツールの利用を行う中核API。JSONスキーマ（`responseSchema`）を定義し、構造化データとメッセージテキストをフロントエンドに返却します。
